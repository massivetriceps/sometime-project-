...
function Navbar() {
  ...
  const handleLogout = async () => {
    try {
      ...
      // 로그아웃 API 실패해도 finally에서 클라이언트 상태 정리
    } finally {
      logout();
      resetCart(); // 장바구니 상태 초기화 — 재로그인 시 새 사용자 카트가 정상 로드되도록
      navigate('/');
      setOpen(false);
    }
  };
  ...
  return (
    <nav
      className="fixed top-0 left-0 right-0 z-50 bg-white/95 backdrop-blur-md border-b border-[#E8F0FF] font-pretendard"
          ...
          {isLoggedIn ? (
            <>
              ...
              <Link to="/mypage" className="flex items-center gap-2 no-underline shrink-0">
                <img
                  src={studentLogo}
                  alt={user?.name ? `${user.name}의 프로필` : '프로필'}
            ...
            <div className="flex items-center gap-1 sm:gap-2">
              <Link
                to="/login"
                className="text-[#1F2937] px-2 py-2 text-xs sm:text-sm font-medium no-underline whitespace-nowrap"
              >
                로그인
              </Link>
              <Link
                  ...
                  <div className="flex flex-col gap-1">
                    ...
                    {[
                      ...
                      <Link
                        ...
                        to={item.to}
                        onClick={() => setOpen(false)}
                        className="px-3 py-3 text-sm text-[#374151] no-underline rounded-xl hover:bg-[#F3F7FF] hover:text-[#4F7CF3] transition-colors"
                      >
                        {item.label}
                      </Link>
                    ))}
...
export default function Layout({ children }) {
  ...
  // 새로고침 후 user가 null이면 토큰으로 사용자 정보 재수화
  useEffect(() => {
    if (!isLoggedIn || user) return;
    api.get('/api/users/me')
      .then(res => {
        if (res.data.resultType === 'SUCCESS') updateUser(res.data.success);
      })
      .catch(() => {});
  ...
  useEffect(() => {
    ...
    api.get('/api/users/me/cart')
      ...
      .catch(() => {});
  }, [isLoggedIn, cartLoaded, setCartFromDB]);
