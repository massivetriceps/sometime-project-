...
export default function Courses() {
  const { cart, addToCart } = useTimetable();
            ...
            <div key={course.id} style={{ background: 'white', borderRadius: 14, border: isInCart(course.id) ? '1px solid #BFD4FF' : '1px solid #E8F0FF', boxShadow: '0 2px 8px rgba(0,0,0,0.04)', padding: '16px 18px' }}>
              <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12 }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 6, flexShrink: 0 }}>
                  {!isInCart(course.id) && (
                    <select value={priority[course.id] || 'medium'} onChange={e => setPriority({ ...priority, [course.id]: e.target.value })}
                      style={{ borderRadius: 8, border: '1px solid #E8F0FF', padding: '6px 8px', fontSize: 12, color: '#6B7280', outline: 'none', ...s }}>
                      <option value="high">최우선</option>
                      <option value="medium">우선</option>
