...
import { GraduationCap, BookOpen, Award, TrendingUp, ArrowRight, CheckCircle2, AlertCircle, Edit } from 'lucide-react';
...
export default function GraduationDashboard() {
  ...
  const remaining3 = {
    majorRequired: CREDIT_DATA.major.required.total - CREDIT_DATA.major.required.completed,
    majorElective: CREDIT_DATA.major.elective.total - CREDIT_DATA.major.elective.completed,
    general: CREDIT_DATA.general.total - CREDIT_DATA.general.completed,
  };
  ...
  const STATS = [
    ...
    { title: '교양', value: `${CREDIT_DATA.general.completed}/${CREDIT_DATA.general.total}`, pct: calcPct(CREDIT_DATA.general.completed, CREDIT_DATA.general.total), icon: <TrendingUp size={16} color="white" />, bg: '#d4a017' },
  ];
    ...
    <div style={{ minHeight: '100vh', background: '#F9FAFB', ...s }}>
      <header style={{ position: 'sticky', top: 0, zIndex: 50, borderBottom: '1px solid #E8F0FF', background: 'rgba(255,255,255,0.95)', backdropFilter: 'blur(8px)' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto', display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: 64, padding: '0 24px' }}>
          ...
          <Link to="/graduation/history" style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: 14, color: '#4F7CF3', textDecoration: 'none', fontWeight: 500 }}>
            <Edit size={13} /> 수강내역 수정
          </Link>
        </div>
      </header>
      ...
      <main style={{ maxWidth: 1100, margin: '0 auto', padding: '32px 24px' }}>
        ...
        <div style={{ marginBottom: 28 }}>
          <h1 style={{ fontSize: 'clamp(24px,4vw,36px)', fontWeight: 700, color: '#1F2937', margin: '0 0 6px' }}>졸업 요건 대시보드</h1>
          <p style={{ fontSize: 16, color: '#6B7280', margin: 0 }}>현재 학점 이수 현황과 졸업까지 남은 학점을 한눈에 확인하세요</p>
        </div>

        {/* Stats Grid — v0 디자인 적용 */}
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: 16, marginBottom: 24 }}>
          {STATS.map((stat, i) => (
            <div key={i} style={{ background: 'white', borderRadius: 16, border: '1px solid #E8F0FF', boxShadow: '0 2px 12px rgba(0,0,0,0.06)', overflow: 'hidden', padding: '20px 20px 16px' }}>
        ...
        <div style={{ background: 'white', borderRadius: 16, border: '1px solid #E8F0FF', boxShadow: '0 2px 12px rgba(0,0,0,0.06)', padding: 24, marginBottom: 24 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12 }}>
            {[
              { label: '전공 필수', value: remaining3.majorRequired, color: '#4F7CF3', bg: '#E8F0FF' },
              { label: '전공 선택', value: remaining3.majorElective, color: '#2EC4B6', bg: '#d1faf5' },
              { label: '교양', value: remaining3.general, color: '#A78BFA', bg: '#ede9fe' },
            ].map((item, i) => (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 28 }}>
          {REQS.map((req, i) => {
            ...
            return (
              <div key={i} style={{ background: 'white', borderRadius: 14, border: done ? '1px solid #bbf7d0' : '1px solid #E8F0FF', boxShadow: '0 2px 8px rgba(0,0,0,0.04)', padding: '18px 20px' }}>
                ...
                <div style={{ height: 8, background: '#F3F4F6', borderRadius: 999 }}>
                  <div style={{ height: 8, borderRadius: 999, width: p + '%', background: done ? '#22c55e' : req.color, transition: 'width 0.8s ease' }} />
                </div>
                {!done && (
