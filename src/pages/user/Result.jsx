...
const AI_COMMENTS = {
  A: '수요일 공강이 확보되었으며, 부족했던 전공 필수 3학점을 채우고 아침 수업을 피한 완벽한 시간표입니다. 월~목 집중 수업 후 금요일 오후 여유로운 마무리가 가능합니다.',
  B: '화요일과 목요일에 수업이 집중되어 월·수·금 공강에 가까운 일정입니다. 동선이 최소화되어 같은 건물 내 이동이 많습니다. 총 이동 거리 최소 플랜입니다.',
  C: '전체적으로 오전 수업이 적고, 수업 간 여유 시간이 충분합니다. AI공학관 이동이 일부 포함되지만 연강이 없어 체력 소모가 적은 시간표입니다.',
};
    ...
    <div style={{ minHeight: '100vh', background: '#F9FAFB', ...s }}>
      <header style={{ position: 'sticky', top: 0, zIndex: 50, borderBottom: '1px solid #E8F0FF', background: 'rgba(255,255,255,0.95)', backdropFilter: 'blur(8px)' }}>
        <div style={{ maxWidth: 896, margin: '0 auto', display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: 64, padding: '0 16px' }}>
          <Link to="/" style={{ display: 'flex', alignItems: 'center', gap: 8, textDecoration: 'none' }}><GachonLogo size={32} /><span style={{ fontSize: 20, fontWeight: 700, color: '#1F2937' }}>Sometime</span></Link>
          <div style={{ display: 'flex', gap: 8 }}>
            <button style={{ display: 'flex', alignItems: 'center', gap: 6, borderRadius: 10, border: '1px solid #E8F0FF', padding: '8px 12px', fontSize: 13, color: '#6B7280', background: 'white', cursor: 'pointer', ...s }}><Download size={14} />저장</button>
            <button style={{ display: 'flex', alignItems: 'center', gap: 6, borderRadius: 10, border: '1px solid #E8F0FF', padding: '8px 12px', fontSize: 13, color: '#6B7280', background: 'white', cursor: 'pointer', ...s }}><Share2 size={14} />공유</button>
          </div>
        </div>
      </header>
